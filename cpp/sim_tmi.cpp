...
namespace sim_tmi
{
    ...
    bool use_cuda_half_svd_for_n(int n)
    {
        // The cuSOLVER half-cut SVD path amortizes well at larger N, but its
        // launch/setup and device-side matrix-construction overhead dominates
        // for the small half-cut matrices generated by N=8 and N=12.  Keep
        // GPU state simulation enabled for those cases, but compute the
        // half-cut Schmidt SVDs with the host LAPACK path.
                                                         ...
                                                         TmiWorkspace &workspace)
    {
        ...
        if (precision == cudaq::SimulationState::precision::fp64)
        {
            if (state.is_on_gpu())
            {
                ...
                if (tensor.data != nullptr && use_cuda_half_svd_for_n(n))
                {
                        ...
                        const std::string &output_path)
    {
        ...
        const bool cuda_half_svd_enabled_for_n = use_cuda_half_svd_for_n(n);
        ...
        std::cerr << "Online TMI simulation\n"
                  ...
                  << ((lapack_runtime::zgesvd() || lapack_runtime::cgesvd())
                          ? (cuda_half_svd_enabled_for_n
                                 ...
                                 : "state-vector Schmidt/SVD for half cuts; GPU cuSOLVER half-cut SVD disabled for N=8/N=12; using host LAPACK SVD for half cuts; BLAS HERK small-cut path when available")
