                    ...
                    bool closed) __qpu__
    {
        ...
        for (std::size_t layer = 0; layer < flayers.size(); ++layer)
        {
            ...
            for (int i = start; i < n - 1; i += 2)
            {
                ...
                {
                    rz(0.78539816339744830962, q[i]); //pi/4 Z rotation, i.e. T gate
                }
                ...
                if (flayers[layer].rot_zz_flags[i]) //pi/2 ZZ rotation (the only four-Majorana interaction)
                {
                    cx(q[i], q[j]);
                    rz(1.57079632679489661923, q[j]);
                    cx(q[i], q[j]);
...
int main(int argc, char *argv[])
{
         ...
         std::strcmp(argv[1], "-h") == 0))
    {
        ...
        std::cout << "  fermion = 0 uses the existing MMS/qubit circuit; fermion = 1 uses parity-preserving fermionic gates and JW FSWAP boundaries; fermion=2 uses fermionic RGS.\n";
    ...
    {
        throw std::invalid_argument("The optional fermion argument must be 0-2.");
    }
    ...
    if (d == 1)
    {
        ...
        if (tmi_output == 1)
        {
            run_1d_tmi(n, periods, realizations, res, p_min, p_max, fermion != 0, fermion >= 2);
        }
        else
        {
            run_1d(n, periods, realizations, res, p_min, p_max, fermion != 0, fermion >= 2);
        }
    }
    else if (d == 2)
    {
