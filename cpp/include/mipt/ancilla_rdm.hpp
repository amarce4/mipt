...
struct Rdm4
{
    ...
    const std::complex<double> &operator()(std::size_t row,
...
inline std::uint64_t embed_environment_bits_four(
    std::uint64_t environment,
...
inline int four_mode_reordered_position(
    int mode,
    const std::array<int, 4> &retained)
{
...
inline double four_mode_fermion_reorder_sign(
    std::uint64_t occupation_mask,
    int n_qubits,
    const std::array<int, 4> &retained)
{
...
template <typename Real>
inline Rdm4 from_host_statevector_four(
    const std::complex<Real> *state,
    int n_qubits,
    const std::array<int, 4> &retained,
    bool fermion_trace)
{
    ...
    for (int local = 0; local < 4; ++local)
    {
        masks[static_cast<std::size_t>(local)] =
            std::uint64_t{1} << retained[static_cast<std::size_t>(local)];
    }
         ...
         ++environment)
    {
        const std::uint64_t base =
            embed_environment_bits_four(environment, retained);
...
inline Rdm4 from_amplitude_queries_four(
    cudaq::state &state,
    int n_qubits,
    const std::array<int, 4> &retained,
    bool fermion_trace)
{
    ...
    const std::size_t amplitude_batch = static_cast<std::size_t>(
        backend::mps_amplitude_batch_size());
    const std::size_t environment_batch =
        std::max<std::size_t>(1, amplitude_batch / 16);
    std::vector<std::vector<int>> basis_states;
    std::vector<double> signs;
    Rdm4 rho;
         ...
         first_environment += environment_batch)
    {
        ...
        for (std::uint64_t offset = 0; offset < count; ++offset)
        {
            const std::uint64_t base = embed_environment_bits_four(
                first_environment + offset, retained);
            for (std::size_t local = 0; local < 16; ++local)
            {
                std::uint64_t index = base;
                for (int bit = 0; bit < 4; ++bit)
...
inline Rdm4 reduced_density_matrix_four(
    cudaq::state &state,
    int n_qubits,
    const std::array<int, 4> &retained,
    bool fermion_trace = false)
{
    ...
    {
        backend::warn_mps_amplitude_path_once(
            "four-ancilla reduced density matrix", n_qubits, false, 0);
        return from_amplitude_queries_four(
            state, n_qubits, retained, fermion_trace);
    }
    ...
    if (state.is_on_gpu())
    {
        ...
        {
            status = fermion_trace
                ? mipt_cuda_rho4_complex_fermion_f64(
                      tensor.data, n_qubits, q0, q1, q2, q3, values)
                : mipt_cuda_rho4_complex_f64(
                      tensor.data, n_qubits, q0, q1, q2, q3, values);
        }
        else
        {
            status = fermion_trace
                ? mipt_cuda_rho4_complex_fermion_f32(
                      tensor.data, n_qubits, q0, q1, q2, q3, values)
                : mipt_cuda_rho4_complex_f32(
                      tensor.data, n_qubits, q0, q1, q2, q3, values);
        }
        if (status != 0)
        {
            throw std::runtime_error(
                "CUDA four-site ancilla reduced-density-matrix kernel failed; status=" +
                std::to_string(status));
        }
        Rdm4 rho;
        for (std::size_t i = 0; i < rho.values.size(); ++i)
    ...
    if (state.is_on_gpu())
    {
        if (precision == cudaq::SimulationState::precision::fp64)
        {
            std::vector<std::complex<double>> host_state(dimension);
            state.to_host(host_state.data(), host_state.size());
            return from_host_statevector_four(
                host_state.data(), n_qubits, retained, fermion_trace);
        }
        std::vector<std::complex<float>> host_state(dimension);
        state.to_host(host_state.data(), host_state.size());
        return from_host_statevector_four(
