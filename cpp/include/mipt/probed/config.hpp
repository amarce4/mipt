    ...
    std::cout << "probe_mode and args:\n"
        ...
        << "  MIPT_PROBED_ISOLATE_P=0      Disable per-p subprocess isolation.\n"
              ...
              << "  MIPT_PROBED_RETRIES=1        Retries after a failed isolated p "
                 "point.\n"
        << "  MIPT_PROBED_BATCHES=64       Two-probe modes 0 and 4 also write a\n"
              << "                               <output>_batches.csv holding this many\n"
              << "                               independent batch-mean copies of the whole\n"
              << "                               (bin, time) grid, so a fit can bootstrap over\n"
              << "                               whole curves instead of resampling points\n"
              << "                               independently against their own stderrs (which\n"
              << "                               understates the error on a collapse exponent).\n"
              << "                               0 disables the sidecar; 50-100 is the useful band.\n"
