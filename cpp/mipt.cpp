...
namespace
{
    ...
    // Store all three-site reduced density matrices for the specified subsystems from the given statevector psi in the output array rho_ri, 
    // which is expected to have space for 2 * 8 * number_of_subsystems doubles. The format of rho_ri is contiguous interleaved real and 
    // imaginary parts, with the first 128 values corresponding to the first subsystem, the next 128 values to the second subsystem, and so on.
        ...
        double *rho_ri)
    {
        ...
        for (std::size_t s = 0; s < subsystems.size(); ++s)
        {
            ...
            std::array<std::uint64_t, RHO3_DIM> retained_offsets{}; // retained_offsets[k] is index on the wider system that corresponds to 
            // reduced-basis index k on the subsystem
            ...
            {
                std::uint64_t base = 0; //base is the index of the environment part of the statevector, assuming the subsystem part is in the |000> state. 
                // The full statevector index is base | retained_offsets[k] for reduced-basis index k.
                int env_bit = 0;
                for (int q = 0; q < n; ++q)
                {
    ...
    // Extract all three-site reduced density matrices for the specified subsystems from the given state.
    // The output format (stored in rho_ri) is a contiguous array of real and imaginary parts interleaved, with the first 128
    // values corresponding to the first subsystem, the next 128 values to the second subsystem, and so on.
...
struct FLayerData //LayerData for fermions
{
    int start = 0;
...
struct MIPTKernel_1D_FermionGateSet
{
    void operator()(int n,
                    const std::vector<FLayerData> &flayers,
                    bool closed) __qpu__
    {
        cudaq::qvector q(n);
...
FLayerData make_fermionic_mipt_layer(int n,
                          int start,
                          double p,
                          std::mt19937 &rng)
{
...
std::vector<FLayerData> fermionic_mipt_frontend(int n,
                                     int periods,
                                     double p)
{
    std::mt19937 rng(std::random_device{}());

    std::vector<FLayerData> layers;
    layers.reserve(2 * periods + 1);

    for (int period = 0; period < periods; ++period)
    {
        layers.push_back(make_fermionic_mipt_layer(n, 0, p, rng)); // even layer
        layers.push_back(make_fermionic_mipt_layer(n, 1, p, rng)); // odd layer
    }

    // Implement final even layer with 50% probability.
    std::bernoulli_distribution coin_flip(0.5);
    if(coin_flip(rng)){
        layers.push_back(make_fermionic_mipt_layer(n, 0, p, rng));
    }
...
void run_1d(int n, int periods, int realizations, int res, double p_min, double p_max, bool fermionic=false)
{
    if (n < RHO3_QUBITS)
    {
    ...
    for (std::size_t p_index = 0; p_index < ps.size(); ++p_index)
    {
        ...
        for (int r = 0; r < realizations; ++r)
        {
            ...
            if(fermionic){
                auto flayers = fermionic_mipt_frontend(n, periods, p);
                if (r == 0)
                {
                    lap1 = std::chrono::steady_clock::now();
                }
                state = cudaq::get_state(MIPTKernel_1D_FermionGateSet{}, n, flayers, true);
            }
            else{
                ...
                state = cudaq::get_state(MIPTKernel_1D{}, n, layers, true);
            }
            ...
            rho3_file.write_record( //Record all three-qubit RDMs for this trajectory, indexed by the current p and r values.
